...
class Dataset(object):
    def __init__(self, datasource, path_data, seed, **kwargs):
        ...
        if self.datasource == 'mnist':
            ...
            self.num_classes = 10
            self.dataset = cifar.read_data(os.path.join(self.path_data, 'cifar-10'))
        elif self.datasource=='cifar-100':
            self.num_classes = 100
            self.dataset = cifar_100.read_data(os.path.join(self.path_data, 'cifar-100'))
        elif self.datasource == 'mnist-fashion':
            self.num_classes=10
            self.dataset = mnist_fashion.read_data(os.path.join(self.path_data, 'MNIST-fashion'))
        else:
