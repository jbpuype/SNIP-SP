...
tf.set_random_seed(777) #if one need
...
class LeNet300(object):
    def __init__(self,
        ...
        self.num_params = sum([helpers.static_size(v) for v in self.weights_ap.values()])
...
class CifarNet(object):
    ...
    def construct_weights(self, initializer_w, initializer_b, trainable, scope):
        ...
        weights = {}
        with tf.variable_scope(scope):
            weights['w1'] = tf.get_variable('w1', [5,5,3,64], **w_params)
            weights['w2'] = tf.get_variable('w2', [5,5,64,64], **w_params)
            weights['b1'] = tf.get_variable('b1', [64], **b_params)
            weights['b2'] = tf.get_variable('b2', [64], **b_params)
            weights['w3'] = tf.get_variable('w3', [4096, 384], **w_params)
            weights['w4'] = tf.get_variable('w4', [384, 192], **w_params)
            weights['w5'] = tf.get_variable('w5', [192, 10], **w_params)
            weights['b3'] = tf.get_variable('b3', [384], **b_params)
            weights['b4'] = tf.get_variable('b4', [192], **b_params)
            weights['b5'] = tf.get_variable('b5', [self.num_classes], **b_params)
        return weights
    ...
    def forward_pass(self, weights, inputs, is_train, trainable=True):
        ...
        inputs = _conv_block(inputs, bn_params, {'w': weights['w1'], 'b': weights['b1']})
        pool1 = tf.nn.max_pool(inputs, [1, 2, 2, 1], [1, 2, 2, 1], 'VALID')
        inputs = _conv_block(pool1, bn_params, {'w': weights['w2'], 'b': weights['b2']})
        inputs = tf.nn.max_pool(inputs, [1, 2, 2, 1], [1, 2, 2, 1], 'VALID')
        inputs = tf.reshape(inputs, [-1, reduce(lambda x, y: x * y, inputs.shape.as_list()[1:])])
        fc1 = tf.matmul(inputs, weights['w3']) + weights['b3']
        fc1 = tf.layers.batch_normalization(fc1, **bn_params)
        fc1 = tf.nn.relu(fc1)
        fc2 = tf.matmul(fc1, weights['w4']) + weights['b4']
        fc2 = tf.layers.batch_normalization(fc2, **bn_params)
        fc2 = tf.nn.relu(fc2)
        fc3 = tf.matmul(fc2, weights['w5']) + weights['b5']
        return fc3